...
from utils.quantization_utils.quant_modules import freeze_model, unfreeze_model
...
def domain_discrepancy(source_feat, target_feat, loss_type="kl"):
    ...
    if loss_type == "kl":
        ...
        for target, source in zip(target_feat, source_feat):
            discrep_loss += kl_div(
                target.detach(), source.detach(), reduction="batchmean"
            )
    elif loss_type == "norm":
        for target, source in zip(target_feat, source_feat):
...
def loss_kd(output, target, teacher_output, args):
    ...
    KD_loss = F.kl_div(
        ...
        reduction="batchmean",
    ) * (alpha * T * T) + criterion(output, target.float()) * (1.0 - alpha)
