...
class ExportQonnxQuantAct(nn.Module):
    ...
    def init_parameters(self):
        ...
        self.scale = self.layer.act_scaling_factor.clone().detach().requires_grad_(False)
        self.node_inputs = (
    ...
    def update_parameters(self, pre_act_scaling_factor):
    ...
    def forward(
        ...
        if pre_act_scaling_factor is None:
            pre_act_scaling_factor = torch.tensor([1], dtype=torch.float32)
        ...
        if self.export_mode:
            ...
            x = self.compute_quant_act(x, pre_act_scaling_factor) * self.scale 
            model_info["quant_out_export_mode"][self.layer] = x
            return (x, self.scale) 
        else:
            # self.update_parameters(pre_act_scaling_factor)  
    ...
    def forward(self, x, prev_act_scaling_factor=None):
        ...
        if self.export_mode:
            ...
            return (
                ...
                model_info["conv_scaling_factor"][self.layer],
            )
        else:
